...
int distMat[MATDIM][MATDIM] = {
    0, 0,  0, 0,  0,  0,  0,  19, 0,  0,  0,  0,  0,  0,  27, 31, 0,
    0, 0,  0, 0,  8,  18, 26, 0,  0,  0,  0,  33, 36, 41, 31, 0,  0,
    0, 18, 1, 32, 17, 35, 0,  0,  13, 13, 29, 14, 28, 12, 0};
...
void matEntryUpdate(int size, int ele1, int ele2) {
  ...
  while (clusterLst[cluster_loc[0]][e1_iter] != -1) {
    ...
    while (clusterLst[cluster_loc[1]][e2_iter] != -1) {
      distSum += distMat[clusterLst[cluster_loc[0]][e1_iter]]
                        [clusterLst[cluster_loc[1]][e2_iter]];
      distCount++;
      e2_iter++;
    }
    e1_iter++;
            ...
            if (i == greaterCombLoc || j == greaterCombLoc)
              ...
              while (clusterLst[itr9_cluster_loc[0]][itr9_e1_iter] != -1) {
                ...
                while (clusterLst[itr9_cluster_loc[1]][itr9_e2_iter] != -1) {
                  itr9_distSum +=
                      distMat[clusterLst[itr9_cluster_loc[0]][itr9_e1_iter]]
                             [clusterLst[itr9_cluster_loc[1]][itr9_e2_iter]];
                  itr9_distCount++;
                  itr9_e2_iter++;
                }
                itr9_e1_iter++;
