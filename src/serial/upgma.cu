...
__global__ void buildUpgma(uint32_t mat_dim, uint32_t *d_distMat,
                           uint32_t *d_opMat, int *d_clusterLst) {
...
// d_distMat[getIndexDev(mat_dim,itr1_j,itr1_i)] = d_distMat[getIndexDev(mat_dim,itr1_i,itr1_j)];
    // itr1_j == bx
    //   itr1_i == tx
    d_distMat[getIndexDev(mat_dim, bx, tx)] =
        d_distMat[getIndexDev(mat_dim, tx, bx)];
  }
  //}
    ...
    for (int k = 0; k < mat_dim - 2; k++) {
        ...
        for (int itr7_j = 0; itr7_j < mat_dim; itr7_j++) {
          if (minLoc[0] == d_clusterLst[getIndexDev(mat_dim, itr7_i, itr7_j)]) {
            ...
            itr7_find_count++;
                     ...
                     d_clusterLst[getIndexDev(mat_dim, itr7_i, itr7_j)]) {
      ...
      if (itr6_cluster_locs[0] != itr6_cluster_locs[1]) {
        ...
        for (itr6_j = 0; itr6_j < mat_dim; itr6_j++) {
          if (d_clusterLst[getIndexDev(mat_dim, itr6_cluster_locs[0],
                                       itr6_j)] == -1)
            break;
        }
        for (int itr6_k = 0; itr6_k < mat_dim; itr6_k++) {
          ...
          if (itr6_ele == -1)
            break;
          d_clusterLst[getIndexDev(mat_dim, itr6_cluster_locs[0], itr6_j)] =
              itr6_ele;
              ...
              if (i == greaterCombLoc || j == greaterCombLoc)
                ...
                for (int itr10_i = 0; itr10_i < mat_dim; itr10_i++) {
                  for (int itr10_j = 0; itr10_j < mat_dim; itr10_j++) {
                    if (i ==
                        d_clusterLst[getIndexDev(mat_dim, itr10_i, itr10_j)]) {
                      itr10_cluster_locs[0] = itr10_i;
                      itr10_find_count++;
                    } else if (j == d_clusterLst[getIndexDev(mat_dim, itr10_i,
                                                             itr10_j)]) {
                ...
                d_opMat[getIndexDev(mat_dim, i, j)] =
                    itr9_distSum / itr9_distCount;
                //}
              }
            }
          }
