...
// Combine the 2 clusters in clusterList that contain ele1 and ele2, IF they belong to separate clusters
void grpLeaf (int** clusterLst, int size, int ele1, int ele2);

// Return an array of 2 elements that represent the location of ele1 and ele2 in the cluster list of lists.
// e.g. [ [0, 3], [1, 5], [2], [4], [6] ]
// if ele1 = 3 and ele2 = 5, the result should be [0,1]
int* findCluster (int** clusterLst, int size, int ele1, int ele2);

// Recompute distances after cluster update
void matEntryUpdate(int** clusterLst, int size, int** d_ref, int** d_red, int ele1, int ele2);
